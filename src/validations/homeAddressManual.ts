...
import { LETTERS, NUMBERS, SPECIAL_CHARS, WHITESPACE } from "./regexParts";

const extemdedOtherAddressDetailsPattern = `^[${LETTERS}${NUMBERS}${WHITESPACE}${SPECIAL_CHARS}]*$`;
...
export const manualAddressValidator = [
    ...
    body("addressPropertyDetails").trim().notEmpty().withMessage("noPropertyDetails").bail()
        .matches(extendedOtherAddressDetailsFormat).withMessage("invalidPropertyDetails").bail()
        .isLength({ max: 200 }).withMessage("invalidPropertyDetailsLength"),

    body("addressLine1").trim().notEmpty().withMessage("noAddressLine1").bail()
        .matches(extendedOtherAddressDetailsFormat).withMessage("invalidAddressLine1").bail()
        .isLength({ max: 50 }).withMessage("invalidAddressLine1Length"),

    body("addressLine2").trim().matches(extendedOtherAddressDetailsFormat).withMessage("invalidAddressLine2").bail()
