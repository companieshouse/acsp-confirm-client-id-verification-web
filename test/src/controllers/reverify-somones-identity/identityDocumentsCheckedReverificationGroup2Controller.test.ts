...
import { REVERIFY_BASE_URL, REVERIFY_WHICH_IDENTITY_DOCS_CHECKED_GROUP2 } from "../../../../src/types/pageURL";
...
describe("GET", () => {
    it("Should display the option 2 eligible list of documents.", async () => {
        const res = await router.get(REVERIFY_BASE_URL + REVERIFY_WHICH_IDENTITY_DOCS_CHECKED_GROUP2);
...
describe("POST", () => {
    it("Redirect to the document details entry page on successfulf submission of the page.", async () => {
        const res = await router.post(REVERIFY_BASE_URL + REVERIFY_WHICH_IDENTITY_DOCS_CHECKED_GROUP2).send({ documentsGroup2A: ["passport", "bankCard"] });
        expect(mocks.mockSessionMiddleware).toHaveBeenCalled();
        expect(mocks.mockAuthenticationMiddleware).toHaveBeenCalled();
    ...
    it("Should advise the validation error when no documents are selected.", async () => {
        const res = await router.post(REVERIFY_BASE_URL + REVERIFY_WHICH_IDENTITY_DOCS_CHECKED_GROUP2).send({ documentsGroup2A: [] });
        expect(mocks.mockSessionMiddleware).toHaveBeenCalled();
        expect(mocks.mockAuthenticationMiddleware).toHaveBeenCalled();
    ...
    it("Should handle the exceptions with an appropiate error page.", async () => {
        ...
        const res = await router.post(REVERIFY_BASE_URL + REVERIFY_WHICH_IDENTITY_DOCS_CHECKED_GROUP2);
        expect(res.status).toBe(500);
        expect(res.text).toContain("Sorry we are experiencing technical difficulties");
    });
