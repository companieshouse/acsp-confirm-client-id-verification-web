...
import { REVERIFY_BASE_URL, REVERIFY_CONFIRMATION } from "../../../../src/types/pageURL";
...
describe("GET " + REVERIFY_BASE_URL + REVERIFY_CONFIRMATION, () => {
    let req;
    beforeEach(() => {
        jest.clearAllMocks();
        req = createRequest({
    ...
    it("should return status 200 and render the confirmation page with formatted data", async () => {
        const res = await router.get(REVERIFY_BASE_URL + REVERIFY_CONFIRMATION + "?lang=en");
        expect(res.status).toBe(200);
        expect(mocks.mockSessionMiddleware).toHaveBeenCalled();
        expect(res.text).toContain("Reverification details submitted");
    });
    ...
    it("should return status 500 if an error occurs", async () => {
        ...
        const res = await router.get(REVERIFY_BASE_URL + REVERIFY_CONFIRMATION);
    ...
    it("should format the client data correctly", async () => {
        ...
        await router.get(REVERIFY_BASE_URL + REVERIFY_CONFIRMATION + "?lang=en");
