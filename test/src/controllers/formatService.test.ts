...
describe("FormatService tests", () => {
    ...
    describe("formatDateIdentityDocuments", () => {
        it("should return an empty array if no document is provided", () => {
            const result = FormatService.formatDateIdentityDocuments();
            expect(result).toEqual([]);
        });

        it("should return an empty array if an empty document array is provided", () => {
            const emptyIdentityDocument: any[] = [];
            const result = FormatService.formatDateIdentityDocuments(emptyIdentityDocument);
            expect(result).toEqual([]);
        });

        it("should format a single document correctly", () => {
            const mockDocument: DocumentDetails[] = [
                {
                    docName: "Biometric or machine readable passport",
                    documentNumber: "123456789",
                    expiryDate: new Date("2025-12-31"),
                    countryOfIssue: "Albania"
                }
            ];

            const result = FormatService.formatDateIdentityDocuments(mockDocument);

            expect(result).toEqual([
                {
                    docName: "Biometric or machine readable passport",
                    documentNumber: "123456789",
                    expiryDate: FormatService.formatDate(new Date("2025-12-31")),
                    countryOfIssue: "Albania"
                }
            ]);
        });
        ...
        it("should format multiple documents correctly", () => {
            const mockDocuments: DocumentDetails[] = [
                ...
                {
                    docName: "Identity card with biometric information from the EU, Norway, Iceland or Liechtenstein",
                    documentNumber: "987654321",
                    expiryDate: new Date("2024-06-30"),
                    countryOfIssue: "Norway"
                }
            ];

            const result = FormatService.formatDateIdentityDocuments(mockDocuments);

            expect(result).toEqual([
                {
                    docName: "Irish passport card",
                    documentNumber: "123456789",
                    expiryDate: FormatService.formatDate(new Date("2025-10-10")),
                    countryOfIssue: "Ireland"
                },
                {
                    docName: "Identity card with biometric information from the EU, Norway, Iceland or Liechtenstein",
                    documentNumber: "987654321",
                    expiryDate: FormatService.formatDate(new Date("2024-06-30")),
                    countryOfIssue: "Norway"
                }
            ]);
        });

        it("should handle missing optional fields (documentNumber and expiryDate)", () => {
            const mockDocument: DocumentDetails[] = [
                {
                    docName: "Biometric or machine readable passport",
                    countryOfIssue: "Wales"
                }
            ];
            ...
            expect(result).toEqual([
                {
                    ...
                    expiryDate: FormatService.formatDate(undefined),
                    countryOfIssue: "Wales"
                }
            ]);
        });
